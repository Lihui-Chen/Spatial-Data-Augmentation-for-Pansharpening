...
def D_lambda_K(fused,ms,ratio,sensor,S):
    ...
    if (fused.shape[0] != (ratio*ms.shape[0]) or fused.shape[1] != (ratio*ms.shape[1])) == 1:
    ...
    fused_degraded = MTF(fused,sensor,ratio, False)
                
    fused_degraded = fused_degraded[int(ratio/2):-1:int(ratio),int(ratio/2):-1:int(ratio),:]

    Q2n_index, Q2n_index_map = q2n(ms,fused_degraded,S,S)
