...
@Contact :   lihuichenscu@foxmail.com
@License :   None
...
def mask_collate_fn_arbrpn(data):
    '''
    ...
    data.sort(key=lambda x: x[0]['GT'].shape[0], reverse=True)
    # data.sort(key=lambda x: x['LR'].shape[0], reverse=True)
    data_length = [sq[0]['GT'].shape[0] for sq in data]
    mask = []
    ...
    for idx, train_data in enumerate(data):
        if pad_size[idx] != 0:
            if train_data[0].get('LR') is not None:
                train_data[0]['LR'] = pad_tensor(train_data[0]['LR'], data_length[0], dim=0)
            if train_data[0].get('MTF') is not None:
                train_data[0]['MTF'] = pad_tensor(train_data[0]['MTF'], data_length[0], dim=0)
            train_data[0]['GT'] = pad_tensor(train_data[0]['GT'], data_length[0], dim=0)
    batch_data = {}
    if data[0][0].get('LR') is not None:
        batch_data['LR'] = torch.stack([train_data[0]['LR'] for train_data in data])
    if data[0][0].get('MTF') is not None:
        batch_data['MTF'] = torch.stack([train_data[0]['MTF'] for train_data in data])
    batch_data['GT'] = torch.stack([train_data[0]['GT'] for train_data in data])
    batch_data['REF'] = torch.stack([train_data[0]['REF'] for train_data in data])
    batch_path = [train_data[1] for train_data in data]
    return batch_data, mask#, batch_path

def mask_collate_fn_trans(data):
    '''
    :param data: batch of samples with different numbers of channels
    :return:
        data:
    ...
    # data, path = data[0][0], data[0][1]
    # data.sort(key=lambda x: x['LR'].shape[0], reverse=True)
    data.sort(key=lambda x: x[0]['LR'].shape[0], reverse=True)
    data_length = [sq[0]['LR'].shape[0] for sq in data]
    mask = torch.zeros((len(data_length), data_length[0]))
    
    for idx, tmp_len in enumerate(data_length):
        mask[idx, :tmp_len] = torch.ones((1, tmp_len))
